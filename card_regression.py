...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        self.model_var_type = config.model.var_type  # fixedlarge
        self.num_timesteps = config.diffusion.timesteps  # 1000
        self.vis_step = config.diffusion.vis_step  # 100
        self.num_figs = config.diffusion.num_figs  # 10
        self.dataset_object = None

        betas = make_beta_schedule(
            schedule=config.diffusion.beta_schedule,  # linear
            num_timesteps=self.num_timesteps,
            start=config.diffusion.beta_start,  # 0.0001
            end=config.diffusion.beta_end,  # 0.02
        )
    ...
    def compute_guiding_prediction(self, x, method="OLS"):
        ...
        if method == "OLS":
            ...
            return x.mm(self.cond_pred_model)
        elif method == "NN":
            return self.cond_pred_model(x)
        else:
            return None
    ...
    def train(self):
        ...
        # Find the best n_epoch and retrain the pre-trained model
        if (
            ...
            if (
                ...
                logging.info(
                    f"\nRe-training the guidance model on original training set with {early_stopper.best_epoch} epochs..."
                )
        ...
        if self.args.resume_training:  # resuming training from saved checkpoint
            states = torch.load(
                    ...
                    if (
                        ...
                        with torch.no_grad():
                            ...
                            for j in range(1, self.num_figs):
                                cur_t = j * self.vis_step
                                cur_y = (
                                    y_p_seq[self.num_timesteps - cur_t].detach().cpu()
                                )
    ...
    def sample(self):
        ...
        with torch.no_grad():
            ...
            for i in range(self.num_figs - 1):
                ...
                axs[i].set_title(
                    "$q(\mathbf{y}_{" + str(i * self.vis_step) + "})$", fontsize=23
                )
            ...
            axs[self.num_figs - 1].set_title(
                "$q(\mathbf{y}_{" + str(self.num_timesteps - 1) + "})$", fontsize=23
            )
            fig.savefig(
                os.path.join(
                    self.args.im_path,
            ...
            for i in range(self.num_figs - 1):
                ...
                axs[1, i].set_title(
                    "$p_{\\theta}(\mathbf{y}_{" + str(i * self.vis_step) + "})$",
                    fontsize=23,
                )
                axs[1, i].set_xlabel(
                    "KL($q(y_t)||p(y_t)$)={:.2f}\nKL($q(y_0)||p(y_t)$)={:.2f}".format(
            ...
            axs[1, self.num_figs - 1].set_title(
                "$p_{\\theta}(\mathbf{y}_{" + str(self.num_timesteps) + "})$",
                fontsize=23,
            )
