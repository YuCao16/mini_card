...
def multiple_experiments(
    ...
    for idx, seed in enumerate(seeds):
        ...
        if idx == model_to_plot:
            model_eval_dict["make_plot"] = True
...
def get_dataset(args, config, test_set=False, validation=False):
    ...
    if config.data.dataset == "swiss_roll":
        ...
        data = Gaussians().sample(
            config.data.dataset_size - config.data.dataset_size % 8, mode=8
        )
    elif config.data.dataset == "25gaussians":
        data = Gaussians().sample(
            config.data.dataset_size - config.data.dataset_size % 25, mode=25
        )
        ...
        data_object = QuadraticDatasetWithOneX(
            a=config.data.a,
            b=config.data.b,
            c=config.data.c,
            n_samples=config.data.dataset_size,
        ...
        data_object = SinusoidDatasetWithOneX(
            n_samples=config.data.dataset_size,
            seed=args.seed,
            x_dict=vars(config.data.x_dict),
            noise_dict=vars(config.data.noise_dict),
        ...
        data_object = CircleDatasetWithOneX(
            r=config.data.r,
            n_samples=config.data.dataset_size,
            seed=args.seed,
            x_dict=vars(config.data.x_dict),
        ...
        data_object = LogLogDatasetWithOneX(
            a=config.data.a,
            b=config.data.b,
            n_samples=config.data.dataset_size,
            seed=args.seed,
